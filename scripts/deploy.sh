...
# Usage: ./scripts/deploy.sh [x.y.z] | ./scripts/deploy.sh --bump build|patch|minor|major
...
ensure_git_clean_and_on_main() {
  ...
  if [[ "$branch" != "main" ]]; then
    echo -e "${RED}❌ Deploy must be run from the 'main' branch (current: '$branch').${NC}" >&2
